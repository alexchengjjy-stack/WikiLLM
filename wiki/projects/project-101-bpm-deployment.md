...
summary: "BZS 替客戶「101」進行 BPM 系統整合建置，包含電子簽章串接與 HiCloud+DMZ 地端部署架構，近期完成 AATL 合約與軌跡紀錄檔名關聯及異常寫回機制規畫。"
---
...
- [ ] **佈署環境架構調整**：HiCloud 與 DMZ 間需新增**轉介 Server**（重新拆分程式碼）
- [ ] **寫回機制異常邏輯**：規劃寫回成功與失敗的系統容錯與重試邏輯（預計 6 月初完成）

### 📅 即將執行
- [ ] 完成 HiCloud + DMZ 轉介 Server 設置
- [ ] 完整 BPM 功能驗收與異常寫回機制測試
- [ ] 系統正式驗收與上線

...
# Eraser.io 霓虹風格 HTML/CSS 架構圖內容定義
html_template = """<!DOCTYPE html>
<html lang="zh-TW">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>BreezyBrain Architecture (Eraser Neon Blueprint)</title>
    <!-- Google Fonts -->
    <link href="https://fonts.googleapis.com/css2?family=Fira+Code:wght@400;500;600;700&family=Inter:wght@300;400;500;600;700;800&family=Outfit:wght@500;600;700;800&family=Noto+Sans+TC:wght@300;400;500;700&display=swap" rel="stylesheet">
    <style>
        :root {
            --bg-color: #080c14;
            --grid-color: rgba(30, 58, 138, 0.15); /* 稍微明亮的藍色格線 */
            ...
            --purple-neon: #b624ff;
            --purple-glow: rgba(182, 36, 255, 0.25);
            
            --orange-neon: #ff6c00;
            --orange-glow: rgba(255, 108, 0, 0.25);
            
            --text-main: #f1f5f9;
            --text-sub: #64748b;
            --card-bg: rgba(9, 15, 28, 0.85);
        }
        ...
        * {
        ...
        .architecture-container {
            ...
            padding: 30px 40px;
            display: grid;
            grid-template-columns: 240px 300px 300px 240px;
        ...
        .layer-presentation {
            ...
            box-shadow: 0 0 20px var(--blue-glow), inset 0 0 12px rgba(0, 210, 255, 0.05);
        }
        .layer-application {
            border: 2px solid var(--green-neon);
            box-shadow: 0 0 20px var(--green-glow), inset 0 0 12px rgba(0, 255, 135, 0.05);
        }
        .layer-brain {
            border: 2px solid var(--purple-neon);
            box-shadow: 0 0 20px var(--purple-glow), inset 0 0 12px rgba(182, 36, 255, 0.05);
        }
        .layer-security {
            border: 2px solid var(--orange-neon);
            box-shadow: 0 0 20px var(--orange-glow), inset 0 0 12px rgba(255, 108, 0, 0.05);
        }
        ...
        .layer-presentation .layer-title { color: var(--blue-neon); }
        .layer-application .layer-title { color: var(--green-neon); }
        .layer-brain .layer-title { color: var(--purple-neon); }
        .layer-security .layer-title { color: var(--orange-neon); }
        ...
        /* 卡片發光邊框 */
        .layer-presentation .node-card { border-left: 3px solid var(--blue-neon); }
        .layer-application .node-card { border-left: 3px solid var(--green-neon); }
        .layer-brain .node-card { border-left: 3px solid var(--purple-neon); }
        .layer-security .node-card { border-left: 3px solid var(--orange-neon); }
        ...
        .layer-presentation .node-icon { color: var(--blue-neon); filter: drop-shadow(0 0 4px var(--blue-glow)); }
        .layer-application .node-icon { color: var(--green-neon); filter: drop-shadow(0 0 4px var(--green-glow)); }
        .layer-brain .node-icon { color: var(--purple-neon); filter: drop-shadow(0 0 4px var(--purple-glow)); }
        .layer-security .node-icon { color: var(--orange-neon); filter: drop-shadow(0 0 4px var(--orange-glow)); }
...
<header>
    <h1>BreezyBrain Architecture</h1>
...
<div class="architecture-container">
    ...
    <svg class="flow-canvas" viewBox="0 0 1366 768">
        <defs>
            ...
            <marker id="arrow-purple" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
                <path d="M 0 1.5 L 8 5 L 0 8.5 z" fill="#b624ff" />
            </marker>
        </defs>

        <!-- Presentation ➡️ Application (CLI Tool 底部引出，分叉指向 CRM、BCR、CLM) -->
        <!-- M: CLI 卡片中心偏右 (X=210, Y=630) H:340 V:分叉對齊 -->
        <path d="M 210 635 H 320" stroke="#00d2ff" stroke-width="2.5" fill="none" filter="url(#glow-blue)" />
        <!-- 分支 1 指向 CRM (Y=175) -->
        <path d="M 320 635 V 175 H 430" stroke="#00d2ff" stroke-width="2.5" fill="none" filter="url(#glow-blue)" marker-end="url(#arrow-blue)" />
        <!-- 分支 2 指向 BCR (Y=285) -->
        <path d="M 320 635 V 285 H 430" stroke="#00d2ff" stroke-width="2.5" fill="none" filter="url(#glow-blue)" marker-end="url(#arrow-blue)" />
        <!-- 分支 3 指向 CLM (Y=510) -->
        <path d="M 320 635 V 510 H 430" stroke="#00d2ff" stroke-width="2.5" fill="none" filter="url(#glow-blue)" marker-end="url(#arrow-blue)" />

        <!-- Application ➡️ AI Brain Core (從 BCR/CLM 右側引出指向 Ollama Router/Qwen LLM) -->
        <!-- 從 CRM 右側 (Y=175) 指向 Ollama Router (Y=220) -->
        <path d="M 750 175 H 800 V 220 H 880" stroke="#00ff87" stroke-width="2.5" fill="none" filter="url(#glow-green)" marker-end="url(#arrow-green)" />
        <!-- 從 CLM 右側 (Y=510) 指向 Qwen LLM (Y=350) -->
        <path d="M 750 510 H 800 V 350 H 880" stroke="#00ff87" stroke-width="2.5" fill="none" filter="url(#glow-green)" marker-end="url(#arrow-green)" />

        <!-- AI Brain Core ➡️ Security & Boundary (從 ReAct Loop 右側引出指向 DMZ Gateway) -->
        <!-- 從 ReAct Loop 右側 (Y=610) 指向 DMZ Gateway (Y=275) -->
        <path d="M 1190 610 H 1220 V 275 H 1250" stroke="#b624ff" stroke-width="2.5" fill="none" filter="url(#glow-purple)" marker-end="url(#arrow-purple)" />
    </svg>
    ...
    <div class="column-wrapper layer-presentation">
        ...
        <div class="node-card">
            ...
            <div class="node-desc">視覺化控制台與審查介面</div>
        </div>
        ...
        <div class="node-card" style="margin-top: auto; margin-bottom: 20px;">
            ...
            <div class="node-desc">地端同步與備份維運 CLI</div>
        </div>
    </div>
    ...
    <div class="column-wrapper layer-application">
        ...
        <div class="node-card">
            ...
            <div class="node-desc">微型客戶資料關係管理</div>
        </div>
        ...
        <div class="node-card">
            ...
            <div class="node-desc">名片拍照 OCR 智慧採集</div>
        </div>
        ...
        <div class="node-card">
            ...
            <div class="node-desc">視覺化審批工作流引擎</div>
        </div>
        ...
        <div class="node-card">
            ...
            <div class="node-desc">合約範本匹配與傳簽派單</div>
        </div>
        ...
        <div class="node-card">
            ...
            <div class="node-desc">完簽合約摘要與知識沈澱</div>
        </div>
    </div>
    ...
    <div class="column-wrapper layer-brain">
        ...
        <div class="node-card">
            ...
            <div class="node-desc">RAG 嵌入與模型降級路由</div>
        </div>
        ...
        <div class="node-card">
            ...
            <div class="node-desc">推理大腦與個資脫敏</div>
        </div>
        ...
        <div class="node-card">
            ...
            <div class="node-desc">向量檢索與語意記憶庫</div>
        </div>
        ...
        <div class="node-card">
            ...
            <div class="node-desc">自動決策與人機確認 (HITL)</div>
        </div>
    </div>
    ...
    <div class="column-wrapper layer-security">
        ...
        <div class="node-card">
            ...
            <div class="node-desc">網路隔離區安全網閘代理</div>
        </div>
        ...
        <div class="node-card">
            ...
            <div class="node-desc">雲端 AATL / LTV 長期憑證簽章</div>
        </div>
        ...
        <div class="node-card">
            ...
            <div class="node-desc">禁止聯外地端降級自簽簽章</div>
        </div>
    </div>
...
<footer>
    <p>BreezyBrain Neon Blueprint ． 2026H2 橫向工作流架構藍圖 (技術討論稿)</p>
</footer>
...
def main():
    ...
    # 定義檔案名稱
    output_html_name = f"{now_str}-breezy-brain-architecture_eraser.html"
    output_pdf_name = f"{now_str}-breezy-brain-architecture_eraser.pdf"
    output_png_name = f"{now_str}-breezy-brain-architecture_eraser.png"
    ...
    # 1. 寫入 HTML 檔案
    print("Writing Eraser.io style architecture HTML file...")
    ...
    try:
        ...
        # 寫入結果供主腳本參照
        with open("outputs/arch_eraser_generation_result.txt", "w", encoding="utf-8") as f:

...
# v6 16:9 橫向 HTML/CSS 架構圖內容定義
# 完美還原 v3 的自適應置中排版、精緻緊湊比例（卡片 padding/gap、字體大小、網格）與垂直數據流箭頭 (workflow-arrow)
# 同時融合 v5 的 SVG 發光圖示、hover 懸停動效與無損導出 PDF/PNG 邏輯
html_template = """<!DOCTYPE html>
<html lang="zh-TW">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>BreezyBrain 產品核心分層架構圖 (v6 16:9 Landscape Blueprint)</title>
    <!-- 引入 Google Fonts -->
    ...
    <style>
        ...
        * {
        ...
        .architecture-container {
            ...
            max-width: 1366px;
            display: grid;
            grid-template-columns: 1fr 1.35fr 1.25fr 1fr; /* 稍微拉寬核心業務直欄 */
            gap: 18px;
        ...
        .column-wrapper {
            ...
            border: 2px solid rgba(255, 255, 255, 0.08); /* 邊框加深 */
            border-radius: 16px;
            padding: 18px;
            box-shadow: 0 12px 30px -5px rgba(0, 0, 0, 0.6), inset 0 1px 1px rgba(255, 255, 255, 0.08);
        ...
        /* 各層發光頂部邊框 */
        .layer-presentation { border-top: 5px solid var(--secondary); box-shadow: 0 0 25px rgba(2, 132, 199, 0.05); }
        .layer-application { border-top: 5px solid var(--primary); box-shadow: 0 0 25px rgba(5, 120, 87, 0.05); }
        .layer-brain { border-top: 5px solid var(--brain); box-shadow: 0 0 25px rgba(124, 58, 237, 0.05); }
        .layer-security { border-top: 5px solid var(--border-glow); box-shadow: 0 0 25px rgba(234, 88, 12, 0.05); }
 
        .layer-title {
            font-family: 'Outfit', 'Noto Sans TC', sans-serif;
            font-size: 22px; /* 放大標題字體 */
            font-weight: 700;
            letter-spacing: 0.5px;
            display: flex;
            align-items: center;
        ...
        /* 模組小卡片 */
        .module-card {
            background: rgba(15, 23, 42, 0.95); /* 更不透明的底色 */
        ...
        /* 模組 SVG 配色 */
        .layer-presentation .module-header svg { color: var(--secondary); filter: drop-shadow(0 0 4px var(--secondary-glow)); }
        .layer-application .module-header svg { color: var(--primary); filter: drop-shadow(0 0 4px var(--primary-glow)); }
        .layer-brain .module-header svg { color: var(--brain); filter: drop-shadow(0 0 4px var(--brain-glow)); }
        .layer-security .module-header svg { color: var(--border-glow); filter: drop-shadow(0 0 4px rgba(234, 88, 12, 0.35)); }
        ...
        .workflow-arrow {
            text-align: center;
            font-size: 14px; /* 放大指向箭頭文字 */
            color: rgba(5, 120, 87, 0.65); /* 提升可見度 */
            margin: -4px 0;
            line-height: 1;
            font-weight: 600;
        }
        ...
        .arrow-indicator {
            ...
            color: rgba(148, 163, 184, 0.35); /* 提高透明度與可見度 */
            z-index: 20;
            pointer-events: none;
        }
...
<header>
    <div class="logo-title">BreezyBrain 16:9 Landscape Blueprint - v6</div>
...
<div class="architecture-container">
    ...
    <div class="column-wrapper layer-presentation">
        ...
        <div class="module-card">
            ...
            <div class="module-desc">為外部與 Agent 提供統一通道。對接標準 MCP 協定，支援 mTLS 安全加密。</div>
            <div class="tech-badge-list">
    ...
    <div class="column-wrapper layer-application">
        ...
        <div class="module-card">
            ...
            <div class="module-desc">對接蒙恬名片雲。支援拍照上傳、名片資訊 OCR 智慧擷取與大腦自動校正。</div>
            <div class="tech-badge-list">
        ...
        <div class="workflow-arrow">▼ 數據流：名片校正 ➡️ CRM 補全</div>
        ...
        <div class="workflow-arrow">▼ 數據流：觸發商機 ➡️ BPM 審批</div>
        ...
        <div class="module-card">
            ...
            <div class="module-desc">Node-based 視覺化工作流編排，結合「人工審批守門員 (HITL)」例外處理。</div>
            <div class="tech-badge-list">
        ...
        <div class="workflow-arrow">▼ 數據流：審批通過 ➡️ CLM 送簽</div>
        ...
        <div class="workflow-arrow">▼ 數據流：完簽 PDF ➡️ KM 歸檔</div>
...
<footer>
    <p>架構圖編號: BZS-ARCH-169-20260529-06 | 蒙恬科技 (PenPower) BreezyBrain 專案開發小組 ． 2026H2 橫向工作流架構藍圖 (技術討論稿)</p>
</footer>
...
def main():
    ...
    # 定義檔案名稱
    output_html_name = f"{now_str}-breezy-brain-architecture_v6.html"
    output_pdf_name = f"{now_str}-breezy-brain-architecture_v6.pdf"
    output_png_name = f"{now_str}-breezy-brain-architecture_v6.png"
    ...
    # 1. 寫入 HTML 檔案
    print("Writing 16:9 landscape architecture v6 HTML file...")
    ...
    print(f"Calling browser: {browser_exe} for headless PNG generation (Forced 1400x1020)...")
    cmd_png = [
        browser_exe,
        "--headless",
        "--disable-gpu",
    ...
    try:
        ...
        # 寫入結果供主腳本參照
        with open("outputs/arch_v6_generation_result.txt", "w", encoding="utf-8") as f:
